...
const INTERFACE_DEST = path.resolve(__dirname, "..", "src", "Command.ts");
...
const concatOptions = options =>
	options.reduce((o, el, i) => o.concat(` "${cleanupString(el)}"${i !== options.length - 1 ? " | " : ""}`), "");
...
const Writer = path => {
	...
	writer.write("export interface Command {\n");
			...
			for (let i = 1; i < keywords.length; i++) {
				...
				if (key) {
					if (key.startsWith("(") && key.endsWith(")")) {
						...
						overload +=
							`, options${optionsIdx > 0 ? optionsIdx : ""}${optionalArgAdded ? "?" : ""}: ` + concatOptions(options);
						++optionsIdx;
						...
						if (str === "function") {
							...
							if (seenConstants.has(str)) {
								overload += `, ${str}${seenConstants.get(str) ? seenConstants.get(str) : ""}${
									optionalArgAdded ? "?" : ""
								}: "${str}"`;
							} else {
...
const getCommandShapes = async (command, url) => {
	const searchCommandSyntax = command => {
		if (command === "fill" || command === "setworldspawn" || command === "spawnpoint" || command === "stopsound") {
			...
			return $2("dd")
				...
				.text()
				.replace(/\([a-zA-Z\s]+\)/g, "");
		} else if (command === "worldborder") {
			// Because gamepedia people are inconsistent and annoying humans whom I wish to give a piece of my mind >.<
			return $("#Syntax")
				.parent()
				.nextAll("ul:nth-of-type(1)")
				.text()
				.replace(/\([a-zA-Z\s]+\)/g, "");
		} else {
			// Works for most command pages.
			return $("#Syntax")
				.parent()
				.nextAll("dl:nth-of-type(1)")
				.text()
				.replace(/\([a-zA-Z\s]+\)/g, "");
		}
	};
...
(async function generate() {
	...
	shapes.forEach(shape => shape.overloads.forEach(o => writer.write(shape.command, o)));
